...
export default function AllocationPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [assetData, allocationData, transferRequestData] = await Promise.all([
    ...
    <div className="p-8 min-h-screen">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Real backend-connected allocation flow for {user?.name || "the signed-in user"}.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
          ...
          {canAllocate ? (
            ...
            <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
              ...
              <p className="mt-2 text-sm text-slate-500">
                You can view allocations, but only Asset Managers and Department Heads can create new ones.
              </p>
            </div>
          )}
        ...
        <div className="space-y-6">
              ...
              {allocations.length === 0 ? (
                ...
                allocations.map((allocation) => {
                  const canReturn =
                    ...
                    (isAdmin ||
                      String(allocation.employeeId) === String(user?.id) ||
                      String(allocation.departmentId) === String(user?.departmentId));
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="text-sm text-slate-500">
                            Holder: {allocation.employee?.name || allocation.department?.name || "Unassigned"} - Status:{" "}
                            {allocation.status}
                          </p>
                        </div>
                        {canReturn && (
          ...
          {(user?.role === "ASSET_MANAGER" || user?.role === "DEPT_HEAD") && (
            <section className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
